...
int maxPWM = 666, alignMaxPWM = 220, fastMaxPWM = 666, slowAlignPWM = 140, mediumAlignPWM = 350, botAlignPWM = 100, rampSlowestPWM = 130;
...
int32_t distance = 0, minDistanceThreshold = 110, maxDistanceThreshold = 270;   //for VL53L0X (TOF)
...
int maxAlignCount = 4;
int TZ1Ticks = -230, TZ2Ticks = -223, TZ3Ticks = -213 ;    //-228, -215, -211
int TZ1ThrowCount = 3, TZ2ThrowCount = 4, TZ3ThrowCount = 5;    //2 4 5
int throwPWM1 = 255, throwPWM2 = 255, throwPWM3 = 205;    //255 215 180
int allBreak = false;
  ...
  if(((isShortPlusOnLeft() && turnFlag == 0) || (isShortPlusOnRight() && turnFlag == 1)) && firstTurn == 0)    ///short junc counter
  {
    if(shortPlusFlag)
    {
      ...
      if(turnFlag == 0)
      {
        ...
        else if((rongBay == 0 && shortPlus == 2) || (rongBay == 1 && shortPlus == 4))   //2 5
        {
          pidAlign(4);
        }
