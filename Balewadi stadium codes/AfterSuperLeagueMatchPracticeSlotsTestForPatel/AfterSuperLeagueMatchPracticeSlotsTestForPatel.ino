/**********************
 * BotAlign Me transfer karna hai....!!!!!!!
changes after 1st slot
1. fast line follow immediately after tz3 throw
2. leaving transfer junction2 speed increased
3. shortPlus reset PROPERLY at junction 4
4. BotAlignPWM increased when both sensors are offline
**********************/
#include <io128.h>
#include <sra128.h>
#include "avr/io.h"
...
//calibrated values for practice slot
int sensorMinFront[numOfSensors] = {418, 1782, 1690, 1584, 1336, 1555, 1460, 2421};
int sensorMinBack[numOfSensors] = {2726, 1354, 1009, 1360, 1991, 246, 551, 2149};
int sensorMinRight[numOfSensors] = {2035, 2148, 2357, 2300, 2380, 2289, 1799, 2203};
int sensorMinLeft[numOfSensors] = {1182, 2061, 1763, 1987, 1643, 1315, 1915, 2240};

int sensorMaxFront[numOfSensors] = {2770, 2906, 2905, 2905, 2888, 2884, 2884, 2885};
int sensorMaxBack[numOfSensors] = {2890, 2889, 2890, 2892, 2905, 2595, 2904, 2907};
int sensorMaxRight[numOfSensors] = {2888, 2887, 2888, 2888, 2873, 2874, 2876, 2876};
int sensorMaxLeft[numOfSensors] = {2890, 2880, 2878, 2879, 2860, 2857, 2855, 2856};



//calibrated in the stadium
//int sensorMinFront[numOfSensors] = {283, 1476, 1438, 1242, 1038, 1324, 1096, 1880};
//int sensorMinBack[numOfSensors] = {2410, 830, 296, 682, 1677, 208, 332, 1827};
//
//int sensorMinRight[numOfSensors] = {1909, 2110, 2320, 2257, 2244, 2180, 1665, 2147};
//int sensorMinLeft[numOfSensors] = {815, 1884, 1571, 1851, 1477, 1122, 1716, 1804};
//
//int sensorMaxFront[numOfSensors] = {2287, 2893, 2894, 2893, 2884, 2884, 2883, 2884};
//
//int sensorMaxBack[numOfSensors] = {2895, 2442, 2060, 2231, 2757, 1313, 1906, 2858};
//
//int sensorMaxRight[numOfSensors] = {2882, 2881, 2882, 2883, 2868, 2869, 2871, 2870};
//int sensorMaxLeft[numOfSensors] = {2804, 2874, 2875, 2875, 2834, 2832, 2830, 2832};



int maxPWM = 666, alignMaxPWM = 220, fastMaxPWM = 666, slowAlignPWM = 190, mediumAlignPWM = 330, botAlignPWM = 100, rampSlowestPWM = 140;
...
int frontPlus = 0, shortPlusNotSure = 0; 

int sLeft, sRight, sFront, sBack;
int32_t distance = 0, minDistanceThreshold = 55, maxDistanceThreshold = 245;   //for VL53L0X (TOF)
...
//
int maxAlignCount = 4;
int TZ1Ticks = -232, TZ2Ticks = -227, TZ3Ticks = -212;    //-228, -215, -213
int TZ1ThrowCount = 3, TZ2ThrowCount = 4, TZ3ThrowCount = 5;    //2 4 5
int throwPWM1 = 255, throwPWM2 = 255, throwPWM3 = 203;    //255 215 180
int allBreak = false;
  ...
  else if(bit_is_clear(PING,0))
  {
    ...
    while(1)
    {
      ...
      if(bit_is_clear(PINA,7))
      {
        for(int i = 0; i<8; i++)
        {
          sensorMinLeft[i]  = 4000;
          sensorMaxLeft[i]  = 0;
          sensorMinRight[i] = 4000;
          sensorMaxRight[i] = 0;
          
          sensorMinFront[i]  = 4000;
          sensorMaxFront[i]  = 0;                        
          sensorMinBack[i] = 4000;     
          sensorMaxBack[i] = 0;
        }
        _delay_ms(1000);
...
//        calibrateLeft();
//        calibrateRight();
        Serial.println("Entered Sensor calib");
        calibrateAllSensors();
//        calibrateBack();
        for(int i = 0; i<8; i++)
        {
  ...
  else if(bit_is_clear(PING,3))
  {
    ...
    while(1)
    {
      ...
      else if(bit_is_clear(PING, 0))
      {
        ...
        while(1)
        {
          Serial.print("  TZ1 Ticks = ");Serial.print(TZ1Ticks);
          Serial.print("  TZ2 Ticks = ");Serial.print(TZ2Ticks);
          Serial.print("  TZ3 Ticks = ");Serial.print(TZ3Ticks);
          Serial.print("  TZ3 PWM = ");Serial.print(throwPWM3);
          Serial.println("");
	        if(bit_is_clear(PINA,4))
	        {
	        	while(bit_is_clear(PINA, 4));
...
void calibrateAllSensors()
{
  while(bit_is_set(PING,0))
  {
    ...
    for(int i = 0; i<numOfSensors; i++)
    {
      ...
      Serial.println("Rotate the bot over a junction and press G0 When Done");
      if(sensorMinFront[i] > sensorRawFront[i])
      {
        sensorMinFront[i] = sensorRawFront[i];
      }

      if(sensorMinBack[i] > sensorRawBack[i])
      {
        sensorMinBack[i] = sensorRawBack[i];
      }

      if(sensorMinLeft[i] > sensorRawLeft[i])
      {
        sensorMinLeft[i] = sensorRawLeft[i];
      }

      if(sensorMinRight[i] > sensorRawRight[i])
      {
        sensorMinRight[i] = sensorRawRight[i];
      }


      if(sensorMaxFront[i] < sensorRawFront[i])
      {
        sensorMaxFront[i] = sensorRawFront[i];
      }

      if(sensorMaxBack[i] < sensorRawBack[i])
      {
        sensorMaxBack[i] = sensorRawBack[i];
      }

      if(sensorMaxLeft[i] < sensorRawLeft[i])
      {
        sensorMaxLeft[i] = sensorRawLeft[i];
      }

      if(sensorMaxRight[i] < sensorRawRight[i])
      {
        sensorMaxRight[i] = sensorRawRight[i];
      }
    }
  }
...
void botAlign(int currentDir)
{
  ...
  switch(currentDir)
  {
    ...
    if(frontOnline == 0 || backOnline == 0)
    {
      turnFlag = 1;
      botAlignPWM = 170;
      pidAlign(2);
      while(frontOnline == 0 || backOnline == 0)
      {
        calcAllError();
        calcSpeedToMotors();
        uploadSpeedLineFollow(maxErrorPWM);
...
void shortJuncCheck()
{
  if(((isShortPlusOnLeft() && turnFlag == 0) || (isShortPlusOnRight() && turnFlag == 1)) && firstTurn == 0)    ///short junc counter
  {
    ...
    if(shortPlusFlag)
    {
      ...
      if(turnFlag == 0)
      {
        if((rongBay == 0 && shortPlus == 2) || (rongBay == 1 && shortPlus == 5))    //1 4
        {
          pidAlign(1);      //220
        }
        else if((rongBay == 0 && shortPlus == 1) || (rongBay == 1 && shortPlus == 1))
        {
          flapOpen();
        }
      }
  ...
  if(turnFlag == 0)
  {
    if(shortPlus == 1 || shortPlus == 3 || shortPlus == 5) 
    {
      PORTA |= (1 << PA0);    //LED ON
    }
    else
  ...
  {
    if(shortPlus == 0 || shortPlus == 2 || shortPlus == 4)
    {
      PORTA |= (1 << PA0);    //LED ON
    }
    else
...
//        Serial.println(distance);
      ...
      throwUnbrake();   //for braking the throwing motor
...
//        Serial.println(distance);
  ...
  else if(juncCount == 4 && turnFlag == 1 && rongBay == 1)
  {
    if(shortPlus>=3 || shortPlus <=1)
    {
      shortPlus = 2;
    }
  }
