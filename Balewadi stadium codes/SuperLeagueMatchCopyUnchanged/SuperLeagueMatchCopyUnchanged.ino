...
5. G3 throw test
//int sensorMinFront[numOfSensors] = {307, 1571, 1475, 1330, 1093, 1358, 1199, 2149};
**********************/
#include <io128.h>
#include <sra128.h>
#include "avr/io.h"
...
//calibrated individually for min values
//int sensorMinFront[numOfSensors] = {1844, 1373, 1181, 1430, 296, 540, 1216, 353};
//int sensorMinBack[numOfSensors] = {2614, 1112, 679, 1117, 1868, 220, 415, 1993};
//int sensorMinRight[numOfSensors] = {1898, 2042, 2273, 2198, 2205, 2123, 1609, 2118};
//int sensorMinLeft[numOfSensors] = {1293, 2109, 1817, 2061, 1689, 1394, 1941, 2268};
//int sensorMaxFront[numOfSensors] = {2820, 2811, 2812, 2814, 2568, 2891, 2891, 2878};
//int sensorMaxBack[numOfSensors] = {2889, 2889, 2889, 2890, 2905, 2511, 2904, 2910};
//int sensorMaxRight[numOfSensors] = {2886, 2885, 2886, 2886, 2872, 2873, 2873, 2874};
//int sensorMaxLeft[numOfSensors] = {2882, 2878, 2879, 2878, 2851, 2851, 2850, 2851};



//calibrated in the stadium
int sensorMinRight[numOfSensors] = {1877, 2007, 2249, 2165, 2123, 2145, 1633, 2082};
int sensorMaxRight[numOfSensors] = {2885, 2884, 2885, 2889, 2869, 2870, 2727, 2871};

int sensorMinBack[numOfSensors] = {2489, 1135, 670, 1042, 1805, 218, 405, 1995};
int sensorMaxBack[numOfSensors] = {2890, 2625, 2248, 2335, 2843, 1521, 1993, 2905};

int sensorMinFront[numOfSensors] = {1809, 1386, 1183, 1190, 266, 356, 1144, 304};
int sensorMaxFront[numOfSensors] = {2828, 2427, 2332, 2372, 1280, 1813, 2218, 1577};

int sensorMinLeft[numOfSensors] = {929, 1815, 1537, 1681, 1403, 1115, 1763, 2017};
int sensorMaxLeft[numOfSensors] = {2488, 2885, 2718, 2794, 2568, 2352, 2864, 2869};
...
//
int maxAlignCount = 4;
int TZ1Ticks = -232, TZ2Ticks = -227, TZ3Ticks = -212;    //-232, -227, -212
int TZ1ThrowCount = 3, TZ2ThrowCount = 4, TZ3ThrowCount = 5;    //3 4 5
int throwPWM1 = 255, throwPWM2 = 255, throwPWM3 = 203;    //255 255 203
int allBreak = false;
...
void shortJuncCheck()
{
  if(((isShortPlusOnLeft() && turnFlag == 0) || (isShortPlusOnRight() && turnFlag == 1)) && firstTurn == 0)    ///short junc counter
  {
    ...
    if(shortPlusFlag)
    {
      ...
      if(turnFlag == 0)
      {
        if((rongBay == 0 && shortPlus == 1) || (rongBay == 1 && shortPlus == 4))    //1 4
        {
          pidAlign(3);      //330
        }
