...
int flag_L1 = 0, flag_L2 = 0, flag_R1 = 0, flag_R2 = 0;
int flag_PAD_LEFT = 0, flag_PAD_RIGHT = 0, flag_PAD_UP = 0, flag_PAD_DOWN = 0;
int flag_CROSS = 0, flag_CIRCLE = 0, flag_TRIANGLE = 0, flag_SQUARE = 0;
int flag_SELECT=0,flag_START=0;

int stickLX = 0, stickLY = 0;
int stickRX = 0;
int stickRY = 0;
...
uint8_t psx_button_press(uint16_t button)
{
  uint16_t buttons = psx_buttons();
  return ((buttons & button) > 0);
}

uint8_t psx_button_click(uint16_t button, int &flag)
{
	uint16_t buttons = psx_buttons();
  uint8_t condition = (buttons & button) > 0;
...
{
  psx_read_gamepad();
  stickLX = (psx_stick(PSS_LX)) - 127;
  stickLY = 127 - (psx_stick(PSS_LY));
  stickRX = (psx_stick(PSS_RX)) - 127;
  stickRY = 127 - (psx_stick(PSS_RY));
  if(stickLX == 128 && stickLY == (-128) && stickRX == 128 && stickRY == (-128))
  {
    return (0);
  }
  ...
  if(stickLX > 0)
  {
    if(stickLX <= joystickBuffer)
      stickLX = 0;
    else
      stickLX -= joystickBuffer;
  }
  else if(stickLX < 0)
  {
    if(stickLX >= 0 - joystickBuffer)
      stickLX = 0;
    else
      stickLX += joystickBuffer;
  }
  if(stickRX > 0)
  {
    if(stickRX <= joystickBuffer)
      stickRX = 0;
    else
      stickRX -= joystickBuffer;
  }
  else if(stickRX < 0)
  {
    if(stickRX >= 0 - joystickBuffer)
      stickRX = 0;
    else
      stickRX += joystickBuffer;
  }
  if(stickLY > 0)
  {
    if(stickLY <= joystickBuffer)
      stickLY = 0;
    else
      stickLY -= joystickBuffer;
  }
  else if(stickLY < 0)
  {
    if(stickLY >= 0 - joystickBuffer)
      stickLY = 0;
    else
      stickLY += joystickBuffer;
  }
  if(stickRY > 0)
  {
    if(stickRY <= joystickBuffer)
      stickRY = 0;
    else
      stickRY -= joystickBuffer;
  }
  else if(stickRY < 0)
  {
    if(stickRY >= 0 - joystickBuffer)
      stickRY = 0;
    else
      stickRY += joystickBuffer;
  }
